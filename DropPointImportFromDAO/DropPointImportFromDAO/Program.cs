...
namespace DropPointImportFromGLS
{
    class Program
    {
        ...
        private static string[] zipcodes = {"800",
"900",
"917",
"960",
"999",
...
"9990",};
        ...
        private static AddressHolder ConvertToAddress(dynamic sp)
        {
            ...
            address.DropPointCode = sp["droppoint_id"];
            var add= sp["address"];
            address.Address1 = add["street"];
            address.City = add["city"];
            address.Country = add["country_code"];
            address.Name = sp["name"];
            address.Zip =add["postal_code"];
            address.Latitude = Convert.ToDouble(sp["coordinate"]["latitude"]);
            address.Longitude = Convert.ToDouble(sp["coordinate"]["longitude"]);
            address.SrId = "";
            address.Opening ="";
        ...
        static Dictionary<string, AddressHolder> LoadDropPoints(int Carrier, string Country)
        {
            ...
            foreach (String zip in ziplist)
            {
                try
                {
                    dynamic Address = DAODroppoint(Country,zip);

                    if (Address != null)
                    {
                        Console.Write("run " + zip);
                       
                        foreach (var d in Address)
                        {



                            AddressHolder ah = ConvertToAddress(d);
        ...
        static dynamic DAODroppoint(string country, string zip)
        {

            string uri = "https://api.coolrunner.dk/v1/droppoints/dao";
            string data = string.Format(CultureInfo.InvariantCulture, "country_code={0}&postcode={1}&number_of_droppoints=6", country, zip);
            string response = Post2(uri, data);
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            dynamic respObj = serializer.Deserialize<dynamic>(response);
            //    "result": [
            //{	
            //    "droppoint_id":"6059",
            //    "carrier":"dao",
            ...
            //            "to":"21:00"
            //        },
            if ("ok".Equals(respObj["status"]))
            {
                return respObj["result"];
            }
            Logger.ErrorFormat("Zip not found :{0}", zip);
            return null;
        ...
        static string Post2(string uri, string data)
        {
            string Password = "e8uai7gpuuj3dg1ltvsk3qsrpb65nj6s";
            string User =  "it_request@gtx.nu"; 
            ...
            request.Method = "POST";
            request.Headers.Add("X-Developer-Id", "GTX");
